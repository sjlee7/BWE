...
def generator(self, low, spk=None, name='g_com'):
	...
	n_filter_sizes = [65, 33, 17, 9, 9, 9, 9, 9 ,9, 9]
	...
	# downsampling
		for layer, nf, fs in zip(range(layers), n_filters, n_filter_sizes):
			x = tf.layers.conv1d(x, filters=nf ,kernel_size=fs, strides=2, activation=None, padding='same', kernel_initializer=tf.orthogonal_initializer(gain=1.0)) # cf: fitler, fs:kernel_size
	...
	# bottleneck
		x = tf.layers.conv1d(x, filters=n_filters[-1], kernel_size=n_filter_sizes[-1], strides=2, activation=None, padding='same', kernel_initializer=tf.orthogonal_initializer(gain=1.0))
	...
	# upsampling
		for layer, nf, fs, scl in reversed(list(zip(range(layers), n_filters, n_filter_sizes, down_sampling_layers))):
			x = tf.layers.conv1d(x, filters=nf*2, kernel_size=fs, activation=None, padding='same', kernel_initializer=tf.orthogonal_initializer(gain=1.0))
			# x = tf.layers.batch_normalization(x, training=True)
			x = tf.layers.dropout(x, rate=0.5)
			x = tf.nn.relu(x)
			# print x.shape
